...
plt.savefig(f'./howto_use_csgld_SGLD_distributions_T{temperature}_iter{total_iter}_seed{mySeed}.pdf')
plt.close()
...
for iter_ in range(total_iter):
    ...
    if iter_ % thinning_factor == 0:
        ...
        idx = jax.lax.min(jax.lax.max(jax.lax.floor(energy_value / energy_gap + 1).astype("int32"), 1,), num_partitions - 1,)
...
plt.savefig(f'./howto_use_csgld_CSGLD_distributions_before_T{temperature}_zeta{zeta}_iter{total_iter}_sz{sz}_seed{mySeed}.pdf')
plt.close()
...
top_quantile = 1-5000./len(normalized_energy_pdf)
print(f'pick the most important {(1-top_quantile)*100: .1f}% partitions and ignore the rest')
threshold = jnp.quantile(normalized_energy_pdf, top_quantile)
non_trivial_idx = jnp.arange(num_partitions)[normalized_energy_pdf > threshold]
...
plt.savefig(f'./howto_use_csgld_CSGLD_distributions_after_T{temperature}_zeta{zeta}_iter{total_iter}_sz{sz}_seed{mySeed}.pdf')
plt.close()
